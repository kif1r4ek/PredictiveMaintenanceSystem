...
import type { HistorySource } from "../api/history";
...
interface HistoryTableProps {
  ...
  onSource: (s: HistorySource) => void;
  onRefresh: () => void;
  loading: boolean;
}

export function HistoryTable({ rows, state, source, onSource, onRefresh, loading }: HistoryTableProps) {
  return (
    <div className="card">
      <div className="card__head">
        <div>
          <div className="card__title gap2">
            <Icon name="history" size={16} style={{ color: "var(--fg4)" }} /> Последние прогнозы
          </div>
          <div className="card__sub">Журнал недавних предсказаний</div>
        </div>
        <div className="toolbar">
          <div className="src-toggle">
            <button className={source === "redis" ? "is-on" : ""} onClick={() => onSource("redis")}>Последние</button>
            <button className={source === "db" ? "is-on" : ""} onClick={() => onSource("db")}>Вся история</button>
          </div>
          ...
          <tbody>
              ...
              Array.from({ length: 5 }).map((_, i) => (
                <tr key={i}>
                  {Array.from({ length: 6 }).map((__, j) => (
                    <td key={j}>
                      <div className="sk" style={{ height: 14, width: j === 4 ? "70%" : "55%" }} />
                    </td>
                  ))}
                </tr>
              ))}
            ...
            {state === "done" &&
              rows.map((r) => {
                ...
                const barColor =
                  r.failure_probability < 0.3
                    ? "var(--healthy-600)"
                    : r.failure_probability < 0.6
                    ? "var(--warning-600)"
                    : "var(--critical-600)";
                const ft = failureLabel(r.failure_type);
                return (
                  <tr key={r.id ?? r.created_at}>
                    ...
                    <td>{ft ? <span style={{ color: "var(--fg2)" }}>{ft}</span> : <span className="dash">—</span>}</td>
